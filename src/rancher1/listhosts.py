...
class RancherInstances(object):
    ...
    def __init__(self, servers):
        ...
        for rancher_config in rancher_configs.split():
            ...
            for project in sorted(envstruct["data"], key=getKey):
                ...
                try:
                    ...
                    for instance in sorted(structdata["data"], key=getHostKey):
                        ...
                        instance["check_mk"] = (
                            "https://goldeneye.eea.europa.eu/omdeea/check_mk/index.py?start_url=%2Fomdeea%2Fcheck_mk%2Fview.py%3Fview_name%3Dhost%26host%3D"
            ...
            self.fullText.append(
                "\nUsed RAM {}".format(
                    self.color_percent(
                        (self.fullTotal - self.fullAvailable) * 100 / self.fullTotal
                    )
                )
            )
        ...
        self.content.append(
            "\nAvailable RAM: {:.1f} GiB from a total of {:.1f} GiB".format(
                self.allAvailable / 1024.0, self.allTotal / 1024
            )
        )
        self.content.append(
            "\nUsed RAM {}".format(
                self.color_percent(
                    (self.allTotal - self.allAvailable) * 100 / self.allTotal
                )
            )
        )
    ...
    def write_page(self):
        ...
        server.wiki_page.update(
            self.pageName, project_id=self.projectName, text="\n".join(self.content)
        )
